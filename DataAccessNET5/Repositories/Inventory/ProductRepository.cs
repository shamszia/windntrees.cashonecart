    ...
    public class ProductRepository : EntityRepository<Product>
    {
        ...
        public int GetProductsWithQuantityTotal(SearchInput searchInput)
        {
            ...
            if (searchInput.options != null)
            {
                ...
                if (discountProduct != null)
                {
                    discountQueryPart = "AND [Product].Discount > 0";
                }
            }

            try
            {
                string query = string.Format("SELECT COUNT(0) FROM (SELECT ROW_NUMBER() OVER (ORDER BY ISNULL((SUM(PurchasedQuantity) - SUM(SoldQuantity)),0) DESC) AS RNUMBER,ProductName, PlaceName, ISNULL((SUM(PurchasedQuantity) - SUM(SoldQuantity)),0) AS AvailableQuantity FROM (SELECT [StockPurchased].[Uid], ProductName, PlaceName, ISNULL([Quantity],0) AS PurchasedQuantity, 0 AS SoldQuantity FROM [StockPurchased] INNER JOIN [Product] ON ([StockPurchased].[ProductID] = [Product].[Uid]) INNER JOIN [StockTransaction] ON ([StockTransaction].Uid = [StockPurchased].StockID) WHERE ([Product].Code LIKE @ProductName OR [ProductName] LIKE @ProductName) AND [PlaceName] LIKE @PlaceName AND Category LIKE @Category AND CAST(ISNULL([Top],'') AS NVARCHAR(1)) LIKE @Top AND CAST(ISNULL([Favourite],'') AS NVARCHAR(1)) LIKE @Favourite {0} AND [StockTransaction].Active = 1 UNION SELECT [StockSold].[Uid], ProductName, PlaceName, 0 AS PurchasedQuantity, ISNULL([Quantity],0) AS SoldQuantity FROM [StockSold] INNER JOIN [Product] ON ([StockSold].[ProductID] = [Product].[Uid]) INNER JOIN [SalesTransaction] ON ([SalesTransaction].Uid = [StockSold].StockID) WHERE ([Product].Code LIKE @ProductName OR [ProductName] LIKE @ProductName) AND [PlaceName] LIKE @PlaceName AND Category LIKE @Category AND CAST(ISNUll([Top],'') AS NVARCHAR(1)) LIKE @Top AND CAST(ISNULL([Favourite],'') AS NVARCHAR(1)) LIKE @Favourite {0} AND [SalesTransaction].Active = 1) AvailableStock GROUP BY ProductName, PlaceName) StockList LEFT OUTER JOIN Product ON (Product.Name = ProductName);", discountQueryPart);
        ...
        public List<Product> GetProductsWithQuantity(SearchInput searchInput)
        {
            ...
            try
            {
                //SELECT RNUMBER, ProductName, PlaceName, [Uid],[UserId],[Category],[Manufacturer],[Code],[Color],[Description],[Name],[Picture],[StockLevel],[SalesPrice],[PublishedPrice],[Commission],[Discount],[IncomeTax],[SalesTax],[Favourite],[Top],[RowVersion],AvailableQuantity FROM (SELECT ROW_NUMBER() OVER (ORDER BY ProductName ASC) AS RNUMBER,ProductName, PlaceName, ISNULL((SUM(PurchasedQuantity) - SUM(SoldQuantity)),0) AS AvailableQuantity FROM (SELECT [StockPurchased].[Uid], ProductName, PlaceName, ISNULL([Quantity],0) AS PurchasedQuantity, 0 AS SoldQuantity FROM [StockPurchased] INNER JOIN [Product] ON ([StockPurchased].[ProductID] = [Product].[Uid]) INNER JOIN [StockTransaction] ON ([StockTransaction].Uid = [StockPurchased].StockID) WHERE ([Product].Code LIKE @ProductName OR [ProductName] LIKE @ProductName) AND [PlaceName] LIKE @PlaceName AND [StockTransaction].Active = 1 UNION SELECT [StockSold].[Uid], ProductName, PlaceName, 0 AS PurchasedQuantity, ISNULL([Quantity],0) AS SoldQuantity FROM [StockSold] INNER JOIN [Product] ON ([StockSold].[ProductID] = [Product].[Uid]) INNER JOIN [SalesTransaction] ON ([SalesTransaction].Uid = [StockSold].StockID) WHERE ([Product].Code LIKE @ProductName OR [ProductName] LIKE @ProductName) AND [PlaceName] LIKE @PlaceName AND [SalesTransaction].Active = 1) AvailableStock GROUP BY ProductName, PlaceName) StockList LEFT OUTER JOIN Product ON (Product.Name = ProductName) WHERE Category LIKE @Category AND CAST([Top] AS NVARCHAR(1)) LIKE @Top AND CAST([Favourite] AS NVARCHAR(1)) LIKE @Favourite {0} AND RNUMBER BETWEEN (((@PAGENUMBER - 1) * @PAGESIZE)+1) AND (@PAGENUMBER * @PAGESIZE) ORDER BY AvailableQuantity DESC;
                //SELECT RNUMBER, ProductName, PlaceName, [Uid],[UserId],[Category],[Manufacturer],[Code],[Color],[Description],[Name],[Picture],[StockLevel],[SalesPrice],[PublishedPrice],[Commission],[Discount],[IncomeTax],[SalesTax],[Favourite],[Top],[RowVersion],AvailableQuantity FROM (SELECT ROW_NUMBER() OVER (ORDER BY ISNULL((SUM(PurchasedQuantity) - SUM(SoldQuantity)),0) DESC) AS RNUMBER,ProductName, PlaceName, ISNULL((SUM(PurchasedQuantity) - SUM(SoldQuantity)),0) AS AvailableQuantity FROM (SELECT [StockPurchased].[Uid], ProductName, PlaceName, ISNULL([Quantity],0) AS PurchasedQuantity, 0 AS SoldQuantity FROM [StockPurchased] INNER JOIN [Product] ON ([StockPurchased].[ProductID] = [Product].[Uid]) INNER JOIN [StockTransaction] ON ([StockTransaction].Uid = [StockPurchased].StockID) WHERE ([Product].Code LIKE @ProductName OR [ProductName] LIKE @ProductName) AND [PlaceName] LIKE @PlaceName AND Category LIKE @Category AND CAST(ISNULL([Top],'') AS NVARCHAR(1)) LIKE @Top AND CAST(ISNULL([Favourite],'') AS NVARCHAR(1)) LIKE @Favourite AND [StockTransaction].Active = 1 UNION SELECT [StockSold].[Uid], ProductName, PlaceName, 0 AS PurchasedQuantity, ISNULL([Quantity],0) AS SoldQuantity FROM [StockSold] INNER JOIN [Product] ON ([StockSold].[ProductID] = [Product].[Uid]) INNER JOIN [SalesTransaction] ON ([SalesTransaction].Uid = [StockSold].StockID) WHERE ([Product].Code LIKE @ProductName OR [ProductName] LIKE @ProductName) AND [PlaceName] LIKE @PlaceName AND Category LIKE @Category AND CAST(ISNULL([Top],'') AS NVARCHAR(1)) LIKE @Top AND CAST(ISNULL([Favourite],'') AS NVARCHAR(1)) LIKE @Favourite AND [SalesTransaction].Active = 1) AvailableStock GROUP BY ProductName, PlaceName) StockList LEFT OUTER JOIN Product ON (Product.Name = ProductName) WHERE RNUMBER BETWEEN (((@PAGENUMBER - 1) * @PAGESIZE)+1) AND (@PAGENUMBER * @PAGESIZE);
                string query = string.Format("SELECT RNUMBER, ProductName, PlaceName, [Uid],[UserId],[Category],[Manufacturer],[Code],[Color],[Description],[Name],[Picture],[StockLevel],[SalesPrice],[PublishedPrice],[Commission],[Discount],[IncomeTax],[SalesTax],[Favourite],[Top],[RowVersion],AvailableQuantity FROM (SELECT ROW_NUMBER() OVER (ORDER BY ISNULL((SUM(PurchasedQuantity) - SUM(SoldQuantity)),0) DESC) AS RNUMBER,ProductName, PlaceName, ISNULL((SUM(PurchasedQuantity) - SUM(SoldQuantity)),0) AS AvailableQuantity FROM (SELECT [StockPurchased].[Uid], ProductName, PlaceName, ISNULL([Quantity],0) AS PurchasedQuantity, 0 AS SoldQuantity FROM [StockPurchased] INNER JOIN [Product] ON ([StockPurchased].[ProductID] = [Product].[Uid]) INNER JOIN [StockTransaction] ON ([StockTransaction].Uid = [StockPurchased].StockID) WHERE ([Product].Code LIKE @ProductName OR [ProductName] LIKE @ProductName) AND [PlaceName] LIKE @PlaceName AND Category LIKE @Category AND CAST(ISNULL([Top],'') AS NVARCHAR(1)) LIKE @Top AND CAST(ISNULL([Favourite],'') AS NVARCHAR(1)) LIKE @Favourite {0} AND [StockTransaction].Active = 1 UNION SELECT [StockSold].[Uid], ProductName, PlaceName, 0 AS PurchasedQuantity, ISNULL([Quantity],0) AS SoldQuantity FROM [StockSold] INNER JOIN [Product] ON ([StockSold].[ProductID] = [Product].[Uid]) INNER JOIN [SalesTransaction] ON ([SalesTransaction].Uid = [StockSold].StockID) WHERE ([Product].Code LIKE @ProductName OR [ProductName] LIKE @ProductName) AND [PlaceName] LIKE @PlaceName AND Category LIKE @Category AND CAST(ISNULL([Top],'') AS NVARCHAR(1)) LIKE @Top AND CAST(ISNULL([Favourite],'') AS NVARCHAR(1)) LIKE @Favourite {0} AND [SalesTransaction].Active = 1) AvailableStock GROUP BY ProductName, PlaceName) StockList LEFT OUTER JOIN Product ON (Product.Name = ProductName) WHERE RNUMBER BETWEEN (((@PAGENUMBER - 1) * @PAGESIZE)+1) AND (@PAGENUMBER * @PAGESIZE);", discountQueryPart);
